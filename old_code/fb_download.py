...
##############################################
### Handles Oauth through Temboo ***fix to GAE***
##############################################
...
##############################################
### Get Friends (modify to python facebook SDK)
### ***Looks Deletable***
##############################################
...
def filter_friends(friend_list):
    ...
    for friend in friend_list:
        if type(friend['birthday_date']) == type(u'str'): # add and friend['sex'] != ""
...
#                	friend['age'] *= -1 #Make it negative
    ## **Could then delete next line** ## (But we do need the sex)           
    filtered_list = [friend for friend in friend_list if 'age' in friend and friend['sex'] != ""]
    ## Delete? or separate into female and male lists?
    for friend in filtered_list:
        friend['age_sex'] = str(friend['age'])+friend['sex'][0]
    return filtered_list

##############################################
### Makes Age Sex Strings ***(mod to pos neg)***
##############################################
